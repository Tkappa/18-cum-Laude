...
View::View(){
    ...
    //Inizializza in memoria tutte le finestre della view
...
void View::print_inventory(inventory pg_inventoryfull,int status){
    ...
    //Inventario viene usato sia dal personaggio principale che dal negozio
    //Quindi potrebbe essere che all'ultima chiamata sia stato modificato in modalita negozio, cosi lo riporta allo stato personaggio
    int height=5;
    int width = COLS;
	int startx = 0;
    int starty=LINES-5;
    ...
    //Stampa tutto l'inventario formattato
...
void View::print_outputMap(Map * CurLevel){
    ...
    for(rigMap=0;rigMap<mapHeight;rigMap++){
        for(colMap=0;colMap<mapLenght;colMap++){
            if(CurLevel->roomExplored(colMap,rigMap)){//Stampa la stanza solo in caso sia esplorata
    ...
    //Raccoglie le due liste da stampare
...
void View::print_narrative(queue<char*>* narrativequeue){
    //Stampa tutti gli elementi nella coda narrative
    werase(narrative);
    int cursY=1;
...
void View::print_death(p_char pgprincipale ,p_char omicida){
    ...
    //Messaggio randomico aggiuntivo che spunta alla morte
    ...
    //Se il personaggio non e' stato ucciso da un nemico
    if(omicida==nullptr){
